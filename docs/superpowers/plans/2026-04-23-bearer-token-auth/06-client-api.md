### Task 6：重写客户端 api.ts — localStorage + 自动 refresh
...
// 刷新 token：用旧的 refresh token 换新的 access + refresh token
// 刷新成功后自动更新 localStorage，返回 true
// 刷新失败返回 false（客户端应跳转登录页）
...
- 删除 `getToken()` 方法
- 新增 `setTokens()`、`clearTokens()`、`getAccessToken()` 导出函数
- `request()` 自动带 `Authorization` 头，401 时自动 refresh + 重试
- login/register 返回类型新增 `accessToken`、`refreshToken`
...
Expected: Chat.tsx 等文件可能报错（还没改），api.ts 本身不应报错

- [ ] **Step 3：提交**

```bash
git add client/src/services/api.ts
git commit -m "feat(client): rewrite api.ts - localStorage tokens + auto refresh"
```
